...
def generate_ai_design_critique(
    ...
    """
    On-demand generative reasoning layer (Gemini 3.6 Flash).
    Evaluates layout, materials, and sustainability only when explicitly invoked.
    """
...
def run_intelligent_recommender(room_l: float, room_w: float, budget: float, style: str) -> Dict[str, Any]:
    ...
    return {
        "engine_mode": "Deterministic Spatial Constraint Solver",
        "selected_skus": bundle,
        "total_cost": cost,
        "budget_surplus": surplus,
        "room_area_sqft": area,
        "room_type": room_type_label,
        "design_concept": f"Adaptive {style} {room_type_label} curated for {area} sq ft. Prioritized {len(bundle)} fixtures with verified circulation envelopes."
    }
...
def select_optimal_bundle(catalog, categories, budget, style, min_side):
    ...
    for cat in categories:
        ...
        remaining = len(categories) - len(selected) - 1
        reserve = remaining * 8000
        chosen = next((item for item in candidates if total + item["price_inr"] + reserve <= budget), min(pool, key=lambda x: x["price_inr"]))
...
def solve_spatial_layout(room_l: float, room_w: float, selected_items: List[Dict[str, Any]], door_wall: str = "South") -> Dict[str, Any]:
    ...
    if door_wall == "North":
        ...
        if shower: layout.append({"item_data": shower, "x": margin, "y": margin, "width": shower["width_ft"], "length": shower["length_ft"], "color": "#C5E1A5"})
        if tub:
            tub_x = margin + (shower["width_ft"] if shower else 0.0) + 0.6
            if tub_x + tub["length_ft"] <= room_l - margin: layout.append({"item_data": tub, "x": round(tub_x, 2), "y": margin, "width": tub["length_ft"], "length": tub["width_ft"], "color": "#B3E5FC"})
        if vanity:
            vy = room_w - vanity["length_ft"] - margin - 0.5 if shower else (room_w / 2 - vanity["length_ft"] / 2)
            layout.append({"item_data": vanity, "x": margin, "y": max(margin, round(vy, 2)), "width": vanity["width_ft"], "length": vanity["length_ft"], "color": "#FFE082", "has_faucet": True, "faucet_data": faucet})
        if toilet:
            ty = room_w - toilet["length_ft"] - margin - 0.5 if shower else (room_w / 2 - toilet["length_ft"] / 2)
            layout.append({"item_data": toilet, "x": round(room_l - toilet["width_ft"] - margin, 2), "y": max(margin, round(ty, 2)), "width": toilet["width_ft"], "length": toilet["length_ft"], "color": "#FFCCBC"})
        if acc: layout.append({"item_data": acc, "x": round(room_l - acc["width_ft"] - margin, 2), "y": margin + 0.8, "width": acc["width_ft"], "length": acc["length_ft"], "color": "#E1BEE7"})

    elif door_wall == "South":
        door.update({"x": (room_l - door_w) / 2, "y": 0.0, "dx": door_w, "dy": 0.2})
        if shower: layout.append({"item_data": shower, "x": margin, "y": round(room_w - shower["length_ft"] - margin, 2), "width": shower["width_ft"], "length": shower["length_ft"], "color": "#C5E1A5"})
        if tub:
            tub_x = margin + (shower["width_ft"] if shower else 0.0) + 0.6
            if tub_x + tub["length_ft"] <= room_l - margin: layout.append({"item_data": tub, "x": round(tub_x, 2), "y": round(room_w - tub["width_ft"] - margin, 2), "width": tub["length_ft"], "length": tub["width_ft"], "color": "#B3E5FC"})
        if vanity:
        ...
        if toilet:
            ...
            layout.append({"item_data": toilet, "x": round(room_l - toilet["width_ft"] - margin, 2), "y": max(margin, round(ty, 2)), "width": toilet["width_ft"], "length": toilet["length_ft"], "color": "#FFCCBC"})
        if acc: layout.append({"item_data": acc, "x": round(room_l - acc["width_ft"] - margin, 2), "y": round(room_w / 2, 2), "width": acc["width_ft"], "length": acc["length_ft"], "color": "#E1BEE7"})

    elif door_wall == "East":
        door.update({"x": room_l - 0.2, "y": (room_w - door_w) / 2, "dx": 0.2, "dy": door_w})
        if shower: layout.append({"item_data": shower, "x": margin, "y": round(room_w - shower["length_ft"] - margin, 2), "width": shower["width_ft"], "length": shower["length_ft"], "color": "#C5E1A5"})
        if tub:
            if tub["width_ft"] + (shower["length_ft"] if shower else 3.0) <= room_w: layout.append({"item_data": tub, "x": margin, "y": margin, "width": tub["length_ft"], "length": tub["width_ft"], "color": "#B3E5FC"})
        if vanity: layout.append({"item_data": vanity, "x": round(room_l / 2 - vanity["length_ft"] / 2, 2), "y": round(room_w - vanity["width_ft"] - margin, 2), "width": vanity["length_ft"], "length": vanity["width_ft"], "color": "#FFE082", "has_faucet": True, "faucet_data": faucet})
        if toilet: layout.append({"item_data": toilet, "x": round(room_l / 2 - toilet["width_ft"] / 2, 2), "y": margin, "width": toilet["width_ft"], "length": toilet["length_ft"], "color": "#FFCCBC"})
        if acc: layout.append({"item_data": acc, "x": margin + 0.5, "y": round(room_w / 2, 2), "width": acc["width_ft"], "length": acc["length_ft"], "color": "#E1BEE7"})

    else:  # West
        door.update({"x": 0.0, "y": (room_w - door_w) / 2, "dx": 0.2, "dy": door_w})
        if shower: layout.append({"item_data": shower, "x": round(room_l - shower["width_ft"] - margin, 2), "y": round(room_w - shower["length_ft"] - margin, 2), "width": shower["width_ft"], "length": shower["length_ft"], "color": "#C5E1A5"})
        if tub:
            if tub["width_ft"] + (shower["length_ft"] if shower else 3.0) <= room_w: layout.append({"item_data": tub, "x": round(room_l - tub["length_ft"] - margin, 2), "y": margin, "width": tub["length_ft"], "length": tub["width_ft"], "color": "#B3E5FC"})
        if vanity: layout.append({"item_data": vanity, "x": round(room_l / 2 - vanity["length_ft"] / 2, 2), "y": round(room_w - vanity["width_ft"] - margin, 2), "width": vanity["length_ft"], "length": vanity["width_ft"], "color": "#FFE082", "has_faucet": True, "faucet_data": faucet})
        if toilet: layout.append({"item_data": toilet, "x": round(room_l / 2 - toilet["width_ft"] / 2, 2), "y": margin, "width": toilet["width_ft"], "length": toilet["length_ft"], "color": "#FFCCBC"})
        if acc: layout.append({"item_data": acc, "x": round(room_l - acc["width_ft"] - margin - 0.5, 2), "y": round(room_w / 2, 2), "width": acc["width_ft"], "length": acc["length_ft"], "color": "#E1BEE7"})
